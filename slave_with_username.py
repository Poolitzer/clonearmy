...
myname = '{}'.format(argv[5])
mylink = 'https://t.me/{}'.format(myname)
...
async def await_event(tempclient, event, pre):
    ...
    tempclient.remove_event_handler(hi)
    return message
...
async def autolynch(event, liste, data):
    ...
    response = await await_event2(client, events.NewMessage(from_users=users, chats=group,
                                                            pattern=autopattern),
                                  messageedit)
